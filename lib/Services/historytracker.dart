  ...
  HistoryTracker.addToHistory(this.id, this.imgUrl, this.description, this.name,
        ...
        .then((snapshot) =>
    {
      FirebaseFirestore.instance.collection('Users').doc(uid).collection(
          "History")
          .get().then((snapShots) =>
          snapShots.docs.forEach((s) {
            s.reference.update({
              'index': FieldValue.increment(1)
            });
          })),
      FirebaseFirestore.instance.collection('Users').doc(uid).collection(
          "History").where("index", isGreaterThan: 20).get().then((value) =>
      {
      ...
      if (snapshot.data() == null || !snapshot.exists) {
        ...
        FirebaseFirestore.instance.collection('Users').doc(uid).collection(
            "History").doc(id)
            .update({'click count': FieldValue.increment(1), 'index': 0}),
      }
      else{
          FirebaseFirestore.instance.collection('Users').doc(uid).collection(
              "History").doc(id).get().then((value) =>
          {
            index = value.get('index'),
            FirebaseFirestore.instance.collection('Users').doc(uid).collection(
                "History").where("index", isGreaterThan: index).get().then((
                value) =>
            {
              value.docs.forEach((element) {
                element.reference.update({'index': FieldValue.increment(-1),
                });
              }),
            }),
          },
      ...
      .get().then((snapshots) =>
  {
    snapshots.docs.forEach((productid) {
          ...
          .then((snapshot) =>
      {
        if (snapshot.data() == null || !snapshot.exists) {
          FirebaseFirestore.instance.collection('Users').doc(uid).collection(
              "Purchases").doc(productid.id).set(
              {
